"""
 ...
 DATE: 8/20/17
 USAGE: $ python3 wikipop-starter.py "YOUR SEARCH TERM"
...
# Get the top 5 page results
page_results = wikipedia.search(searchTerm, 5)

# Analyze the resulting pages
for page in page_results:
    ...
    # tokenize each page_content
    tokens = nltk.word_tokenize(page_content)
    # Assign each word with a POS tag
    tagged = nltk.pos_tag(tokens)

    # If the word is an adjective, add it to the return list
    for j,k in tagged:
        if (k=="JJ"):
...
# Get the frequency counts for the adjectives collected
...
# Create the dictionary for the game
